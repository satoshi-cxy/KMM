...
function [laKMM, laMM, BiGraph,isCov, OBJ, TEST1,TEST2,alpha, lambda] = CSBG(X, c, A, k, alpha, lambda)
% Input:
...
for iter = 1:NITER
    ...
    for i = 1:n
        ...
        tmp1(i,:) = EProjSimplex_new(ad);%tmp1是1Xk的行向量
    end
...
function obj = loss(distX,Z,alpha,lambda,U,V )
    ...
    D = spdiags( [ 1./(a1) ;1./(a2')],0,n+m,n+m);
    SS = sparse(n+m,n+m); SS(1:n,n+1:end) = Z; SS(n+1:end,1:n) = Z';
    ft = lambda*trace([U; V]'*(eye(n+m)-D*SS )*[U; V]);
